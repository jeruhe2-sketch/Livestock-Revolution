# -*- coding: utf-8 -*-
"""
(사)한국육류유통수출협회(KMTA) "돼지고기 재고"(부위별 추정 재고량, 톤) 수집.
https://www.kmta.or.kr/kr/info/pork_stock.php
...
응답 표는 부위별(안심/등심/전지/후지/삼겹살/목심/갈비/기타) + 합계 행, 그리고
...
산출: data/kmta_pork_stock.json
  {source, sourceUrl, updatedAt, unit:"톤",
   months: [{label:"2026-06", year, month, parts:{"안심":467,...,"합계":48772}}]}
"""
import json
import os
import sys
...
URL = "https://www.kmta.or.kr/kr/info/pork_stock.php"
OUTPUT_PATH = "data/kmta_pork_stock.json"
...
def fetch_month(session, y, m):
    resp = session.post(
        URL,
        data={"typ": "", "list_url": "/kr/info/pork_stock.php", "page": "1",
              "idate": "", "subitem": "", "board": "info4_0109", "scode": "9",
