...
window.KmtaForeignPriceApp = (function () {
  ...
  return function KmtaForeignPriceApp() {
    ...
    const exportGroupXlsx = () => {
      ...
      const rows = groupItems.map((it) => [it.key, it.v.toFixed(2)]);
      downloadXlsx([header, ...rows], `KMTA_돈육_해외시세_그룹비교_${idxLabel(is)}~${idxLabel(ie)}.xlsx`, "그룹비교");
    };
    const exportOverlayXlsx = () => {
      const header = ["월-주", ...overlay.series.map((s) => s.name)];
      const rows = overlay.categories.map((c, i) => [c, ...overlay.series.map((s) => s.data[i] != null ? s.data[i].toFixed(2) : "")]);
      downloadXlsx([header, ...rows], `KMTA_돈육_해외시세_겹쳐보기.xlsx`, "겹쳐보기");
    };
    ...
    const latestVal = latest ? latest.byCountry?.[headline] ?? null : null;
    const prevVal = prevW ? prevW.byCountry?.[headline] ?? null : null;
    const yearAgoVal = yearAgoW ? yearAgoW.byCountry?.[headline] ?? null : null;
      ...
      mainTab === "chart" && React.createElement(React.Fragment, null,
        ...
        chartSub === "group" && React.createElement("div", { style: { background: COLORS.panel, border: `1px solid ${COLORS.panelBorder}`, borderRadius: 10, padding: 16, marginBottom: 24 } },
          React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, flexWrap: "wrap", gap: 8 } },
            React.createElement("div", { style: { fontSize: 12.5, color: COLORS.mute } }, `\uAC01 \uAD6D\uAC00\uC758 \uC870\uD68C\uAE30\uAC04(${idxLabel(is)}~${idxLabel(ie)}) \uD3C9\uADE0\uAC00\uB97C \uBE44\uAD50\uD569\uB2C8\uB2E4.`),
          ...
          groupItems.length
            ? React.createElement(BarRanking, { items: groupItems, formatValue: (v) => `$${v.toFixed(2)}` })
        ...
        chartSub === "overlay" && React.createElement("div", { style: { background: COLORS.panel, border: `1px solid ${COLORS.panelBorder}`, borderRadius: 10, padding: 16, marginBottom: 24 } },
          ...
          overlay.series.length && overlay.categories.length
            ? React.createElement(React.Fragment, null,
                React.createElement(SvgLineChart, { categories: overlay.categories, series: overlay.series, formatAxisValue: (v) => `$${v.toFixed(2)}` }),
