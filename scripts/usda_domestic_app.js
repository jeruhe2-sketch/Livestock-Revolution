...
window.UsdaDomesticApp = (function () {
  ...
  const C={bg:'#151312',panel:'#1d1a19',border:'#2b2624',amber:'#d98b3f',amberSoft:'#e8b877',cream:'#f2ead9',mute:'#8f857a',sage:'#6f9482'};
  ...
  function Chart({rows}){
    ...
    const lo=Math.max(0,Math.min(...vals)*.95),hi=Math.max(...vals)*1.05,span=Math.max(.01,hi-lo),x=i=>P.l+(rows.length<2?iw/2:i*iw/(rows.length-1)),y=v=>P.t+ih-(v-lo)/span*ih;
    const colors=[C.amber,C.sage,'#4f8fb8'];
    return React.createElement('div',{style:{overflowX:'auto'}},React.createElement('svg',{viewBox:`0 0 ${W} ${H}`,style:{width:'100%',minWidth:680,height:H,display:'block'}},
      ...
      rows.map((r,i)=>i%Math.max(1,Math.ceil(rows.length/10))===0?React.createElement('text',{key:r.date,x:x(i),y:H-12,textAnchor:'middle',fontSize:9,fill:C.mute},r.date.slice(0,7)):null),
      ...
      rows.length<=120&&ITEMS.flatMap((item,j)=>rows.map((r,i)=>{const v=Number(r[item.key]?.usdPerLb);return Number.isFinite(v)?React.createElement('circle',{key:`${j}-${i}`,cx:x(i),cy:y(v),r:2.2,fill:colors[j]}):null;}))
  ...
  function App(){
    const [db,setDb]=useState(null),[err,setErr]=useState(null),[days,setDays]=useState(1095),[tab,setTab]=useState('chart');
    ...
    if(err)return React.createElement('div',{style:{background:C.bg,color:'#c2695f',minHeight:'100vh',padding:30}},`미국 축산물 내수 현황 오류: ${err}`);
    ...
    return React.createElement('div',{style:{background:C.bg,color:C.cream,minHeight:'100vh',padding:'20px 16px 40px'}},React.createElement('div',{style:{maxWidth:1120,margin:'0 auto'}},
      ...
      React.createElement('div',{style:{background:C.panel,border:`1px solid ${C.border}`,borderRadius:10,padding:'10px 14px',marginBottom:12,fontSize:11,color:C.mute}},`최근 발표 ${date(cur?.date)} · 데이터 ${date(db.period?.start)} ~ ${date(db.period?.end)} · USDA AMS MyMarketNews · PK602`),
      ...
      tab==='chart'&&React.createElement(React.Fragment,null,React.createElement('div',{style:{display:'flex',justifyContent:'flex-end',gap:5,marginBottom:8,flexWrap:'wrap'}},[[1095,'3년'],[730,'2년'],[365,'1년'],[180,'6개월'],[90,'3개월']].map(([d,label])=>React.createElement('button',{key:d,onClick:()=>setDays(d),style:{padding:'5px 10px',background:C.panel,border:`1px solid ${days===d?C.amber:C.border}`,borderRadius:7,color:days===d?C.amber:C.mute,cursor:'pointer'}},label))),React.createElement('div',{style:{background:C.panel,border:`1px solid ${C.border}`,borderRadius:12,padding:16}},React.createElement(Chart,{rows})),React.createElement('div',{style:{fontSize:10.5,color:C.mute,marginTop:10}},'※ USDA 원자료 Wtd Avg는 $/100 lb입니다. 예: $145.00/100 lb = $1.45/lb.')),
